        ...
        NavigationStack {
            VStack(spacing: 20) {
                ...
                HStack {
                    ...
                    Menu {
                        NavigationLink("Edit alert text", destination: MessageListView())
                        NavigationLink("Load saved timer", destination: TemplateView())
                        NavigationLink("Settings", destination: UserSettingView())
                    } label: {
                ...
                // TODO: 메인타이머 시간을 초과하는 알림은 설정할 수 없는 에러처리
                PreTimerSelectorViewModel(
                    alertTimeOptions: alertTimeOptions,
                    selectedAlertTimes: $selectedAlertTimes,
                    isSettingMode: isSettingMode,
                    toggleAlertTime: toggleAlertTime
                )
